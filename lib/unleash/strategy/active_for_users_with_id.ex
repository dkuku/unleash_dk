defmodule Unleash.Strategy.ActiveForUsersWithId do
  ...
  @impl Strategy
  @doc false
  def enabled?(%{"userIds" => list}, %{user_id: user_id}) when is_binary(list) and is_binary(user_id) do
    {Utils.in_list?(list, user_id), %{user_id_list: list, user_id: user_id}}
  end

  @impl Strategy
  @doc false
  def enabled?(%{"userIds" => list}, %{user_id: user_id}) when is_binary(list) and is_integer(user_id) do
